...
var _ = NimbusDescribe("Netflix-Redis service", func() {

	var appName, redisName string

	BeforeEach(func() {

		if Config.GetIncludeNimbusServiceNetflixRedis() != true {
			Skip("include_nimbus_service_netflix_redis was not set to true")
		}

		appName = random_name.CATSRandomName("APP")
		redisName = random_name.CATSRandomName("SVC")

		Expect(cf.Cf("create-service", Config.GetNimbusServiceNameNetflixRedis(), "shared-vm", redisName).Wait(Config.DefaultTimeoutDuration())).To(Exit(0))
